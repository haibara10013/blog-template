...
export default function ProfileCard({ postCount, chatterCount, momentsCount, photoCount }: { postCount: number, chatterCount: number, momentsCount: number, photoCount: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-4 md:gap-4 relative z-10">
        <div className="flex gap-2 sm:gap-5 md:gap-4 w-full md:w-auto justify-between sm:justify-around md:justify-start px-2 sm:px-0 md:shrink-0">
          <StatItem count={postCount} label="文章" color="text-indigo-600 dark:text-indigo-400" onClick={(e) => { e.stopPropagation(); router.push('/timeline'); }} />
          <div className="w-px h-8 md:h-10 bg-slate-300/50 dark:bg-slate-700 hidden md:block shrink-0"></div>
          <StatItem count={chatterCount} label="杂谈" color="text-purple-600 dark:text-purple-400" onClick={(e) => { e.stopPropagation(); router.push('/chatter'); }} />
          <div className="w-px h-8 md:h-10 bg-slate-300/50 dark:bg-slate-700 hidden md:block shrink-0"></div>
          <StatItem count={momentsCount} label="说说" color="text-cyan-600 dark:text-cyan-400" onClick={(e) => { e.stopPropagation(); router.push('/moments'); }} />
          <div className="w-px h-8 md:h-10 bg-slate-300/50 dark:bg-slate-700 hidden md:block shrink-0"></div>
          <StatItem count={photoCount} label="照片" color="text-pink-600 dark:text-pink-400" onClick={(e) => { e.stopPropagation(); router.push('/photowall'); }} />
        </div>

        <div className="flex gap-2 md:gap-3 flex-wrap md:flex-nowrap justify-center md:justify-end w-full md:w-auto" onClick={(e) => e.stopPropagation()}>
...
function StatItem({ count, label, color, onClick }: { count: number, label: string, color: string, onClick?: (e: React.MouseEvent) => void }) {
  return (
    <div className="text-center group/stat px-2 cursor-pointer" onClick={onClick}>
